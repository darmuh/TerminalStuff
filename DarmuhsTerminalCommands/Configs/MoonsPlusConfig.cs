...
public class MoonsPlusConfig
{
    ...
    public static ConfigEntry<FilterView.SortingStyle> DefaultSorting { get; internal set; } = null!;
    ...
    internal static void Init()
    {
        ...
        DefaultSorting = MakeGeneric(Plugin.instance.Config, "MoonsPlus", "DefaultSorting", FilterView.SortingStyle.ID_Up, "Set the default sorting style, vanilla is \"id\"");
        ...
        MenuStartPage = MakeGeneric(Plugin.instance.Config, "MoonsPlus", "MenuStartPage", StartPage.Main, "The menu that will open when first running the moons command.");
        ...
        FilterView.Sorting = DefaultSorting.Value;
        FilterView.SetDefaults(DefaultDisplayStyle.Value);
        AcceptableWeathers = [.. WeathersToKeep.Value.Split(',')];
        AcceptableWeathers = [.. AcceptableWeathers.ConvertAll(x => x.Trim().ToLowerInvariant())];

        Loggers.LogDebug("MoonsPlus config section done");
    }
}
