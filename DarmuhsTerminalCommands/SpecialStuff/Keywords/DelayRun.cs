...
internal static class HandleDelayRun
{
    ...
    internal static string HandleCommandDelay()
    {
        ...
        if (command.Length < 1)
        {
            Loggers.LogDebug("No given words after keyword!");
            return failText;
        }
        ...
        else
        {
            ...
            if (words.Length < 2)
            {
                Loggers.LogDebug($"words count is not enough! [ {words.Length} ]");
                return failText;
            }

            Loggers.LogDebug($"words length - {words.Length}");
            string givenCommand = string.Join(" ", words, 1, words.Length - 1);

            if (!int.TryParse(words[0], out int delay))
            {
                Loggers.LogDebug($"Unable to parse value from [ {words[0]} ]");
                return failText;
            }
            else
            {
...
internal class DelayRun
{
    ...
    internal IEnumerator CommandRunner()
    {
        ...
        {
            Loggers.LogInfo($"{command} delayed by {delay} seconds canceled due to detected ship phase change!");
            yield break;
        }

        if (Plugin.instance.Terminal.placeableObject.inUse)
        {
            Loggers.LogInfo($"{command} delayed by {delay} seconds canceled due to terminal in use at this moment!");
            yield break;
        }

        if (!enabled)
        {
            Loggers.LogInfo($"{command} delayed by {delay} seconds canceled due to manual disable!");
            yield break;
        }
